...
class Player(Entity):
	def __init__(self,pos,groups,obstacle_sprites,create_attack,destroy_attack,create_magic):
		...
		# stats
		self.stats = {'health': 1000,'energy':1000,'attack': 10,'magic': 4,'speed': 5}
		self.max_stats = {'health': 300, 'energy': 300, 'attack': 20, 'magic' : 10, 'speed': 10}
	...
	def import_player_assets(self):
		...
		self.animations = {'up': [],'down': [],'left': [],'right': [],
			...
			'right_attack':[],'left_attack':[],'up_attack':[],'down_attack':[],'left_cast':[],'right_cast':[]}
	...
	def input(self):
		if not self.attacking:
			...
			if keys[pygame.K_n] and self.hitbox.centerx >= 1024 and self.hitbox.centery >= 1024 and self.hitbox.centerx <= 1152 and self.hitbox.centery <= 1152 and self.point >= 1200:
				self.point = 0
			elif keys[pygame.K_SPACE] and not self.casting:
				self.attacking = True
			...
			# magic input 
			if keys[pygame.K_LCTRL] and not self.attacking and not self.casting:
				self.casting = True
				self.attack_time = pygame.time.get_ticks()
	...
	def get_status(self):
		...
		if self.direction.x == 0 and self.direction.y == 0:
			if not 'idle' in self.status and not 'attack' in self.status and not 'cast' in self.status:
